...
//				System.out.println("i="+i+" j="+j);
				if (i > 16) {
					// if row number is greater than 16, test only right
					// horizontal
					if (j < 17) {
						// no test required for greater than 16
						tempRight = getProduct(grid[i][j], grid[i][j + 1], grid[i][j + 2], grid[i][j + 3]);
						tempLarge = Math.max(tempLarge, tempRight);
					}
				} else if (j < 3) {
					// if column is less than 3, test for right horizontal,
					// right diagonal, down
					tempRight = getProduct(grid[i][j], grid[i][j + 1], grid[i][j + 2], grid[i][j + 3]);
					tempDiaRight = getProduct(grid[i][j], grid[i+1][j + 1], grid[i+2][j + 2], grid[i+3][j + 3]);
					tempDown = getProduct(grid[i][j], grid[i+1][j], grid[i+2][j], grid[i+3][j]);
					
					tempLarge = Math.max(tempLarge, tempRight);
					tempLarge = Math.max(tempLarge, tempDiaRight);
					tempLarge = Math.max(tempLarge, tempDown);
					
				} else if (j > 16) {
					// if column is greater than 16, test for left diagonal,
					// down
					tempDiaLeft = getProduct(grid[i][j], grid[i + 1][j - 1], grid[i + 2][j - 2], grid[i + 3][j - 3]);
					tempDown = getProduct(grid[i][j], grid[i+1][j], grid[i+2][j], grid[i+3][j]);
					
					tempLarge = Math.max(tempLarge, tempDiaLeft);
					tempLarge = Math.max(tempLarge, tempDown);
					
				} else {
					// else test for right horizontal, right diagonal, left
					// diagonal, down
					tempRight = getProduct(grid[i][j], grid[i][j + 1], grid[i][j + 2], grid[i][j + 3]);
					tempDiaRight = getProduct(grid[i][j], grid[i+1][j + 1], grid[i+2][j + 2], grid[i+3][j + 3]);
					tempDiaLeft = getProduct(grid[i][j], grid[i + 1][j - 1], grid[i + 2][j - 2], grid[i + 3][j - 3]);
			...
			largestProd = Math.max(largestProd, tempLarge);
		}
		System.out.println(largestProd);
	}
	
	private static long getProduct(int a, int b, int c, int d){
//		System.out.print("Product of "+a+"*"+b+"*"+c+"*"+d+" = ");
		long result = a*b*c*d;
