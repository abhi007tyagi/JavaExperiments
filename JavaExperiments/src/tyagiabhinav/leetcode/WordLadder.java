...
public class WordLadder {
    ...
    // working but slow
    public static int bruteLadderLength(String beginWord, String endWord, List<String> wordList) {
        if (!wordList.contains(endWord)) return 0;
        // create map here for word with 1 char diff
        ...
        for (int i = 0; i < wordList.size(); i++) {
            ...
            for (int j = i + 1; j < wordList.size(); j++) {
                if (getDiff(wordList.get(i), wordList.get(j)) == 1)
                    tq.add(wordList.get(j));
            }
            map.put(wordList.get(i), tq);
        }
        ...
        while (!q.isEmpty()) {
            ...
            for (int i = 0; i < size; i++) {
                String t = q.poll();
                seen.add(t);
                if (getDiff(t, endWord) == 1)
                    return ++count;
                for (String w : wordList) {
                    if (!seen.contains(w) && getDiff(t, w) == 1)
                        q.add(w);
                }
            }
            count++;
    ...
    public static int ladderLength(String beginWord, String endWord, List<String> wordList) {
        ...
        for (int i = 0; i < wordList.size(); i++) {
            if (getDiff(beginWord, wordList.get(i)) == 1)
                tq.add(wordList.get(i));
            map.put(beginWord, tq);
        }
    ...
    private static int getDiff(String a, String b) {
        ...
        for (int i = 0; i < a.length(); i++) {
            if (b.charAt(i) != a.charAt(i)) count++;
...
//        List<String> list = Arrays.asList(new String[]{"slit", "bunk", "wars", "ping", "viva", "wynn", "wows", "irks",
...
//        System.out.println(ladderLength("sand", "acne", list));
